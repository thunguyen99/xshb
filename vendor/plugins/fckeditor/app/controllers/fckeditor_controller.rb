...
class FckeditorController < ActionController::Base
  ...
  def current_directory_path
    base_dir = "#{RAILS_ROOT}/public/images/assets"
  ...
  def upload_directory_path
    uploaded = ActionController::Base.relative_url_root.to_s+"/images/assets#{params[:uploaded]||UPLOADED}/#{params[:Type]}"
  ...
  def check_path(path)
    ...
    if exp_path !~ %r[^#{File.expand_path(RAILS_ROOT)}/public/images/assets#{params[:uploaded]||UPLOADED}]
      @errorNumber = 403
      throw Exception.new
    end
    path
